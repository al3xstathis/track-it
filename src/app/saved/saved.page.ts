...
export interface EventId extends EventInterface {
    id: string;
}
            ...
            buttons: [
                ...
                },
                {
                    ...
                    handler: async data => {
                        ...
                        this.savedCollection = this.db.collection('users').doc(id)
                            .collection<EventInterface>('saved', ref => ref.where('type', '==', 'fHep'));
                        this.events = this.savedCollection.snapshotChanges().pipe(
                            map(actions => actions.map(a => {
                                const data = a.payload.doc.data() as EventInterface;
                                const id = a.payload.doc.id;
                ...
                },
                {
                    ...
                    handler: async data => {
                        ...
                        this.savedCollection = this.db.collection('users').doc(id)
                            .collection<EventInterface>('saved', ref => ref.where('type', '==', 'mHep'));
                        this.events = this.savedCollection.snapshotChanges().pipe(
                            map(actions => actions.map(a => {
                                const data = a.payload.doc.data() as EventInterface;
                                const id = a.payload.doc.id;
                ...
                },
                {
                    ...
                    handler: async data => {
                        ...
                        this.savedCollection = this.db.collection('users').doc(id)
                            .collection<EventInterface>('saved', ref => ref.where('type', '==', 'fPent'));
                        this.events = this.savedCollection.snapshotChanges().pipe(
                            map(actions => actions.map(a => {
                                const data = a.payload.doc.data() as EventInterface;
                                const id = a.payload.doc.id;
